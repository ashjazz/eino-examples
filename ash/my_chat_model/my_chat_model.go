...
import (
	"context"
	"errors"
	"github.com/cloudwego/eino-examples/ash/my_options"
...
func NewMyChatModel(config *MyChatModelConfig) (*MyChatModel, error) {
	if config.APIKey == "" {
		return nil, errors.New("chat model apiKey is required")
	}
...
func (m *MyChatModel) Generate(ctx context.Context, messages []*schema.Message, opts ...model.Option) (*schema.Message, error) {
	// 1、处理选项
	options := &my_options.MyChatModelOptions{
...
func (m *MyChatModel) doGenerate(ctx context.Context, messages []*schema.Message, opts *my_options.MyChatModelOptions) (*schema.Message, error) {
	// 实现生成逻辑
	return nil, nil
}

func (m *MyChatModel) doStream(ctx context.Context, messages []*schema.Message, opts *my_options.MyChatModelOptions, sr *schema.StreamWriter[*model.CallbackOutput]) {
	// 流式生成本文写入sr中
	return
}
